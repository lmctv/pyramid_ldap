...
class _LDAPQuery(object):
    ...
    query results. The search string will be obtained by replacing the
    ``${<name>}`` placeholders in the query string with the values 
    of the corresponding keyword args to the execute method.

    See :py:func:`ldap_set_login_query` and :py:func:`ldap_set_groups_query`
    for a more thorough discussion of the class parameters.
    """
    def __init__(self, base_dn, filter_tmpl, scope, cache_period,
                  search_after_bind):
        self.base_dn = base_dn
        self.base_dn_templater = Template(base_dn)
        self.filter_tmpl = filter_tmpl
        self.filter_templater = Template(filter_tmpl)
        self.scope = scope
    ...
    def execute_cache(self, conn, *cache_key):
    ...
    def execute(self, conn, **kw):
        if not self.filter_tmpl:
            result = [(self.base_dn_templater.substitute(**kw), {})]
            logger.debug('result generated by string templating: %r' % result)
            return result
        else:
            cache_key = (
                bytes_(self.base_dn, 'utf-8'),
                self.scope,
                bytes_(self.filter_templater.substitute(**kw), 'utf-8')
                )
        return self.execute_cache(conn, *cache_key)
...
def ldap_set_login_query(config, base_dn, filter_tmpl='',
    ...
    be used as an LDAP filter: it should contain the placeholder ``${login}``
    which will be replaced in login searches.

    If ``filter_tmpl`` is empty, ``base_dn`` will be used as a template for
    building a syntetic search result entry, with just a DN and no attributes.

    ``scope`` is any valid LDAP scope value (e.g. ``ldap.SCOPE_ONELEVEL``).

    ``cache_period`` is the number of seconds to cache login search results;
    if it is 0, login search results will not be cached. 

    ``search_after_bind``, if true, will force a base-scoped search
    for the user entry after a successful bind.  This useful both in the
    case of direct bind with a templated dn, and for directory setups
    that severely limited the access to entry attributes for users different
    by the one represented in the directory by the entry.

    Examples::

        # Standard "search before login" setup
        ...
        # templated "direct access" setup
        config.set_ldap_login_query(
            base_dn='sAMAccountName=${login},CN=Users,DC=example,DC=com',
            filter_tmpl=''
            scope=ldap.SCOPE_ONELEVEL,
    ...
    a valid login.
    """
    query = _LDAPQuery(base_dn, filter_tmpl, scope, cache_period, search_after_bind)
    def register():
...
def ldap_set_groups_query(config, base_dn, filter_tmpl='',
    ...
    not be cached. The ``search_after_bind`` parameter is ignored for group
    searches.
    ...
    """
    # Force the search_after_bind parameter to False in calling _LDAPQuery's
    # constructor.
    query = _LDAPQuery(base_dn, filter_tmpl, scope, cache_period, False)
    def register():
...
def ldap_setup(config, uri, bind='', passwd='', pool_size=10, retry_max=3,
